= checkers
== Description
A BASH script for OS X and Linux (coming soon) that checks the availability of websites and APIs, providing notifications via on screen display and voice. Example:
...
== Rational
- I want the alert of a site down before any user, but I don't check my email every minute
- running this script locally every minute usually beats other monitors (Nagios, monit, etc) that rely on being emailed
- any site down popup is 'sticky' so it will stay there until you acknowledge it, *which is annoying*, so it gets your attention - which is good for the ADD in all of us!
